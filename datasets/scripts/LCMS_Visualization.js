...
Example script for visualizing LCMS change summaries
A more in-depth visualization of LCMS products is available at: https://apps.fs.usda.gov/lcms-viewer/
...
var names_values_colors = {
  ...
  'Land_Use' : {'names':[
    ...
    'Change' : {'names':[
                    ...
                    'colors':['3d4551','f39268','d54309','00a398',"1B1716"]
              }};
...
var lcms = ee.ImageCollection('projects/lcms-292214/assets/Final_Outputs/2020-5/LCMS');
print('Available study areas:',lcms.aggregate_histogram('study_area').keys());
print('Available LCMS products:',lcms.first().bandNames());
print('See https://apps.fs.usda.gov/lcms-viewer/ for a more in-depth visualization of LCMS products');
/////////////////////////////////////////////////////////////////////////////////
...
var earlyLU = LU.filter(ee.Filter.calendarRange(earlySpan[0],earlySpan[1],'year')).mode();
var lateLU = LU.filter(ee.Filter.calendarRange(lateSpan[0],lateSpan[1],'year')).mode();
Map.addLayer(earlyLU,luViz,'Land Use Mode ('+earlySpan.join('-')+')',false);
Map.addLayer(lateLU,luViz,'Recent Land Use Mode ('+lateSpan.join('-')+')',false);
/////////////////////////////////////////////////////////////////////////////////
//Visualize Land Cover change
var LC = lcms.select(['Land_Cover']);
var earlyLC = LC.filter(ee.Filter.calendarRange(earlySpan[0],earlySpan[1],'year')).mode();
var lateLC = LC.filter(ee.Filter.calendarRange(lateSpan[0],lateSpan[1],'year')).mode();
Map.addLayer(earlyLC,lcViz,'Early Land Cover Mode ('+earlySpan.join('-')+')',false);
Map.addLayer(lateLC,lcViz,'Recent Land Cover Mode ('+lateSpan.join('-')+')',false);
/////////////////////////////////////////////////////////////////////////////////
