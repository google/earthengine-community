# Earth Engine geospatial agent for above ground biomass estimation.
...
The agent has a set of tools intended to support above ground biomass
estimation in polygons provided as GeoJSON through the chat interface.
...
2.  **Installation example**
    ...
    *   Copy the files `.env`, `agent.py`, `tools.py`, `prompts.py`, and 
    `__init__.py` to a local directory called `biomass_estimation_agent`
...
```bash
adk run biomass_estimation_agent
```
...
Select `biomass_estimation_agent` from the dropdown.
