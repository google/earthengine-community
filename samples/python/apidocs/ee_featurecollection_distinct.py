...
fc = ee.FeatureCollection('WRI/GPPD/power_plants').filter(
    'country_lg == "Belgium"')
display('FeatureCollection of power plants in Belgium:', fc.getInfo())

# Remove duplicate features according to property values.
display('Distinct based on a single property:', fc.distinct('fuel1').getInfo())
display('Distinct based on two properties:',
        fc.distinct(['fuel1', 'source']).getInfo())
display('Distinct based on geometry', fc.distinct('.geo').getInfo())
