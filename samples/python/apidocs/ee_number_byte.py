...
display('Number cast to byte:', byte_number.getInfo())
...
display('Floating point value cast to byte:', float_to_byte.getInfo())

# A number greater than byte range max cast to byte becomes byte range max.
BYTE_MAX = 255
out_of_range_hi = ee.Number(BYTE_MAX + 12345)
display('Greater than byte max:', out_of_range_hi.getInfo())
...
display('Less than byte min:', out_of_range_lo.getInfo())
