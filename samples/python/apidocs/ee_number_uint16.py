...
display('Number cast to uint16:', uint16_number.getInfo())
...
display('Floating point value cast to uint16:', float_number_to_uint16.getInfo())

# A number greater than uint16 range max cast to uint16
# becomes uint16 range max.
UINT16_MAX = 65535
out_of_range_hi = ee.Number(UINT16_MAX + 12345)
display('Greater than uint16 max:', out_of_range_hi.getInfo())
...
display('Less than uint16 min:', out_of_range_lo.getInfo())
