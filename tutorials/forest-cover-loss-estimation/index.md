...
---
[Open In Code Editor](https://code.earthengine.google.com/5a0328dfb96e3e86ad390e0cd6ed5059)
...
10% and minimum forest area of 0.5 ha (the national definition may be
different).

```js
// Enter a country name and set parameters for forest definition.
// County name must match an entry in Large Scale International Boundary (LSIB)
// dataset.
var country = 'Bolivia';  // selected country (e.g. Bolivia)

// Enter minimum canopy cover percentage (e.g. 10%) for forest definition.
var cc = ee.Number(10);

// Enter minimum forest size in pixels for forest definition (e.g. 6 pixels,
// approximately 0.5 ha in this example)
var pixels = ee.Number(6);

// Enter minimum mapping area for tree loss (usually same as the minimum forest
// area)
...
Depending on the location of the country, the number of pixels that make up for
...
2\. Apply the minimum canopy cover percentage (e.g. greater than or equal to
10%). Use `selfMask()` to mask value zero

```js
var canopyCover10 = canopyCover.gte(cc).selfMask();
```

3\. Apply the minimum area requirement using `connectedPixelCount` (e.g. greater
than or equal to 6 pixels). Note that if no kernel is passed to
...
Map.addLayer(
    minArea.reproject(prj.atScale(scale)), {palette: ['#96ED89']},
...
// Select the tree loss within the derived tree cover (>= canopy cover and area
requirements).
...
1\. Use the derived tree cover and tree loss from the previous steps<br>
...
Map.addLayer(
    minArea01.reproject(prj.atScale(scale)), {palette: ['#168039']},
...
[GOFC-GOLD 2013 A sourcebook of methods and procedures for monitoring and reporting anthropogenic greenhouse gas emissions and removals associated with deforestation, gains and losses of carbon stocks in forests remaining forests, and forestation p 126 GOFC-GOLD Report (version COP22-1)](http://www.gofcgold.wur.nl/redd/sourcebook/GOFC-GOLD_Sourcebook.pdf)
